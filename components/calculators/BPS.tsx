...
const LiabilityRow: React.FC<LiabilityRowProps> = React.memo(
  ({ item, onChange, onRemove, isCbbl, loanTypes }) => {
    return (
        ...
        <div className="w-14 flex items-center justify-between shrink-0 pl-1">
          <div className="flex flex-col items-center leading-none">
            ...
            <label
              ...
              className={`font-medium cursor-pointer text-[9px] ${(isCbbl && item.isTopUp) || (!isCbbl && item.isTakeover) ? "text-green-600 dark:text-green-500" : "text-slate-400 dark:text-slate-500"}`}
            >
              {isCbbl ? "TopUp" : "Take"}
            </label>
          </div>
...
const BPSLoanEligibilityCalculator = () => {
  ...
  const addLiability = useCallback((isCbbl: boolean) => {
    const newItem: Omit<Liability, "isTakeover" | "isTopUp"> & {
      isTakeover?: boolean;
      isTopUp?: boolean;
    } = {
      id: Date.now(),
        ...
        liabilities.map((item) => {
          ...
          if (updatedItem.loanType === "CARD") {
            if (isCbbl) {
              ...
              const outstanding =
                parseFloat(String(updatedItem.outstanding)) || 0;
              updatedItem.emi = Math.round(
                Math.max(limit * 0.03, outstanding * 0.05),
              );
            }
          }
          return updatedItem;
  ...
  const removeLiability = useCallback(
    (isCbbl: boolean, id: string | number) => {
  ...
  const { upToEmi, tableData } = useMemo(() => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      rows.push({
        ...
        amount: displayAmt,
        emi:
          typeof displayAmt === "number"
            ? calculateEmi(displayAmt, rate, i)
            : 0,
      });
    }
  ...
  const handleOtherChange = useCallback(
    (id: string | number, f: string, v: string | boolean) =>
      updateLiability(false, id, f, v),
    [updateLiability],
  );
  const handleCbblChange = useCallback(
    (id: string | number, f: string, v: string | boolean) =>
      updateLiability(true, id, f, v),
    [updateLiability],
  );
  const handleOtherRemove = useCallback(
    (id: string | number) => removeLiability(false, id),
    [removeLiability],
  );
  const handleCbblRemove = useCallback(
    (id: string | number) => removeLiability(true, id),
    [removeLiability],
  );
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-sm p-3 max-w-2xl mx-auto border-t-4 border-blue-500 dark:border-blue-800 animate-fade-in">
      <h2 className="text-lg font-bold text-blue-900 dark:text-blue-300 mb-1 flex items-center gap-2">
        <Briefcase className="text-blue-600 dark:text-blue-400" size={20} /> BPS
        Loan Eligibility
      </h2>

      <div className="flex items-center gap-2 mb-3">
        <label className="text-red-500 dark:text-red-400 text-xs font-medium">
      ...
      <div className="mb-1">
        <div className="flex justify-between items-center mb-1">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[10px] bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-zinc-800 dark:text-slate-200 dark:hover:bg-zinc-700 px-2 py-0.5 rounded"
          >
            <Plus size={10} /> Add
          </button>
        </div>
        ...
        <div className="flex items-center gap-px px-0 mb-2 text-xs">
          <div className="flex-grow min-w-[50px] text-right font-bold text-slate-400 dark:text-slate-500 pr-2">
            Total
          </div>
      ...
      <div className="bg-white dark:bg-zinc-900/50 p-1 rounded-lg mb-4 text-xs border border-blue-200 dark:border-zinc-700 shadow-sm flex items-center gap-1">
        <div className="flex-grow min-w-[60px] pl-2 flex items-center">
          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-tight mr-1.5">
            Available EMI:
          </span>
          <span className="font-bold text-emerald-600 dark:text-emerald-500 text-sm">
            {formatNum(upToEmi)}
          </span>
          <span className="text-[9px] text-slate-400 dark:text-slate-500 ml-0.5 font-medium">
            BDT
          </span>
        </div>
        <div className="w-16 shrink-0"></div>
        <div className="w-20 shrink-0 flex flex-col justify-center bg-slate-50 dark:bg-zinc-800 rounded border border-slate-200 dark:border-zinc-700 py-1 px-1">
          <span className="text-[7px] text-slate-400 dark:text-slate-500 uppercase leading-none text-right mb-0.5">
            Total Limit
          </span>
          <span className="font-bold text-slate-700 dark:text-slate-200 text-right leading-none">
            {formatNum(totalStats.limit)}
          </span>
        </div>
        <div className="w-20 shrink-0 flex flex-col justify-center bg-slate-50 dark:bg-zinc-800 rounded border border-slate-200 dark:border-zinc-700 py-1 px-1">
          <span className="text-[7px] text-slate-400 dark:text-slate-500 uppercase leading-none text-right mb-0.5">
          ...
          <span className="font-bold text-slate-700 dark:text-slate-200 text-right leading-none">
            {formatNum(totalStats.outstanding)}
          </span>
        </div>
        <div className="w-20 shrink-0 flex flex-col justify-center bg-red-50 dark:bg-red-900/20 rounded border border-red-100 dark:border-red-500/30 py-1 px-1">
          <span className="text-[7px] text-red-400 dark:text-red-500 uppercase leading-none text-right mb-0.5">
            Total EMI
          </span>
          <span className="font-bold text-red-600 dark:text-red-400 text-right leading-none">
            {formatNum(totalStats.emi)}
          </span>
        </div>
        <div className="w-14 shrink-0 flex justify-center">
          <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider text-center">
            Total
          </span>
        </div>
      </div>
        ...
        <div className="overflow-hidden rounded-xl border dark:border-zinc-700 shadow-md mb-4">
          <table className="w-full text-xs text-left border-collapse">
            <thead className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white">
              <tr>
                <th className="px-2 py-1.5 font-semibold tracking-wide border-b border-blue-800 text-center">
                  Tenor
                </th>
                <th className="px-2 py-1.5 font-semibold tracking-wide border-b border-blue-800 text-right">
                  Max Loan
                </th>
                <th className="px-2 py-1.5 font-semibold tracking-wide border-b border-blue-800 text-right">
                  Monthly EMI
            ...
            <tbody className="divide-y divide-blue-50 dark:divide-zinc-800">
              {tableData.map((row, idx) => (
                <tr
                  key={idx}
                  className={`transition-colors hover:bg-blue-100/50 dark:hover:bg-zinc-800/50 ${idx % 2 === 0 ? "bg-white dark:bg-zinc-900" : "bg-blue-50/30 dark:bg-zinc-800/30"}`}
                >
                  <td className="px-2 py-1 font-medium text-slate-700 dark:text-slate-300 text-center">
                    {row.tenor} Years
                  </td>
